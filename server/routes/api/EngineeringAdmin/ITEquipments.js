...
router.post('/:id/maintenance', upload.single('file'), async (req,res)=>{
  try {
      // Handle file upload
      if (req.file) {
        req.body.MaintenanceImage = 'http://10.0.1.23:5000/uploads/itEquipments/' + req.file.filename ;
        // return res.status(400).json({ error: 'No file uploaded.' });
      }
    const updatedITEquipment = await itEquipmentInfo.findByIdAndUpdate(req.params.id, { $push: { MaintenanceDtls: req.body } }, { new: true });
...
router.put('/:id/remove', async (req,res)=>{
  try {
   ...
   // res.json( toDeleteEquiptment)
      const deletedITEquipment = await itEquipmentInfo.findByIdAndUpdate(req.params.id,toDeleteITEquiptment,{new:true});
...
router.put('/:id/maintenance/remove/:cid', async (req,res)=>{
  try {
    ...
    const EquipmentsIndex = Equipments.MaintenanceDtls.findIndex(sub => sub._id == cid);
...
router.get('/perequipment', async (req, res) => {
  try {
    const AllEquipments = await itEquipmentInfo.find({},{_id:0,EquipmentType:1});
